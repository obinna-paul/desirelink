 ...
 * over precomputed inputs (CreatorAffinity.affinity, PostQuality.quality). No Prisma import
 ...
 *
 * v1 deliberately omits the plan's topicMatch term (would be 0.20 of the weight) - there's
 * no HashtagTopic/PostTopic mapping yet to compute it from, and it is NOT renormalized away,
 * so a fully-scored post tops out around 0.80 minus any penalty. The three remaining weights
 * keep the plan's original values so relative ordering between affinity/quality/recency
 * stays what the plan intended.
 */
...
export function scorePost(input: RankablePost, now: Date = new Date()): ScoreBreakdown {
  ...
  const score = SCORE_WEIGHTS.affinity * affinity + SCORE_WEIGHTS.quality * quality + SCORE_WEIGHTS.recency * recency - penalty;

  return { score, affinity, quality, recency, penalty };
}
