...
export async function isPremiumUser(profileId: string): Promise<boolean> {
  ...
  return Boolean(
    ...
    premium.currentPeriodEnd > new Date(),
  );
}
...
export async function getDailyMessageUsage(profileId: string) {
  ...
  const [directMessages, groupMessages] = await Promise.all([
    prisma.message.count({
      where: { senderId: profileId, createdAt: { gte: since } },
    }),
...
export async function getDailyProviderPostUsage(profileId: string) {
  ...
  try {
    ...
    console.warn(
      "Provider post usage is unavailable until EngagementMetric migrations are applied.",
    );
  }
  ...
  const viewedPostIds = new Set(
    metrics
      .map((metric) =>
        metric.metricType.slice(PROVIDER_POST_VIEW_METRIC_PREFIX.length),
      )
      .filter(Boolean),
  );
...
export async function recordProviderPostView(
  providerId: string,
  ...
  try {
    ...
    console.warn(
      "Provider post view tracking is unavailable until EngagementMetric migrations are applied.",
    );
  }
}

const POST_VIEW_METRIC_PREFIX = "post_view:";
...
export async function recordPostView(
  ...
  if (authorId === viewerProfileId) return;

  const metricType = `${POST_VIEW_METRIC_PREFIX}${postId}`;
  try {
    const existing = await prisma.engagementMetric.findFirst({
      where: { providerId: authorId, userId: viewerProfileId, metricType },
      select: { id: true },
    });
    if (existing) return;
    ...
    console.warn(
      "Post view tracking is unavailable until EngagementMetric migrations are applied.",
    );
  }
}
...
export async function recordProfileVisit(
  ...
  try {
    ...
    console.warn(
      "Profile visit tracking is unavailable until EngagementMetric migrations are applied.",
    );
  }
}
...
export async function getProfileViewerList(profileId: string) {
  ...
  try {
    ...
    console.warn(
      "Profile viewer list is unavailable until EngagementMetric migrations are applied.",
    );
    return [];
  }
...
export async function countActivePremiumSubscriptionsInRange(
  start: Date,
  end: Date,
): Promise<number> {
...
export async function subscribeToPremium(
  ...
  const customerId = await getOrCreatePaymentCustomerId(
    profileId,
    profile.paymentCustomerId,
  );
  ...
  if (defaultCard) {
    const { reference, success } =
      await paymentProvider.chargeSavedPaymentMethod(
        customerId,
    ...
    if (!success) {
      return {
        ...
        status: 402,
        error:
          "Your saved card was declined. Try updating your payment method.",
      };
    }
    ...
    await prisma.transaction.create({
      data: {
        userId: profileId,
        amountCents: PREMIUM_SUBSCRIPTION_PRICE_CENTS,
        status: "succeeded",
        provider: "card",
        isPremium: true,
      },
