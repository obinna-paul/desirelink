...
const AUTH_PATHS = new Set(["/landing", "/login", "/signup"]);
...
export function PwaInstallPrompt() {
  ...
  useFocusTrap(showIosGuide, dialogRef);

  useEffect(() => {
    if (!isAuthScreen || !isMobileDevice() || isStandaloneMode()) return;
  ...
  if (!isAuthScreen || !platform || dismissedForVisit || isStandaloneMode()) return null;
        ...
        >
          ...
          >
            ...
            <span className="mt-0.5 block text-xs leading-4 text-white/70">
              {platform === "ios" ? "Add it to your Home Screen." : installing ? "Opening installer..." : "Install the app on your phone."}
            </span>
          </button>
          <button
            type="button"
