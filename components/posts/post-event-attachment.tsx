...
export function PostEventAttachment({ event }: { event: PostEventView }) {
  ...
  return (
    <section className="rounded-2xl border border-border bg-accent-tint/40 p-3 md:rounded-lg">
      <div className="flex flex-wrap items-center gap-1.5">
        <Badge variant="outline" className="label-caps">{event.eventType}</Badge>
        <Badge variant={event.priceCents > 0 ? "secondary" : "tint"} className="label-caps">
          {event.priceCents > 0 ? formatCents(event.priceCents) : "Free"}
        </Badge>
        {isFull && <Badge variant="outline" className="label-caps">Full</Badge>}
      </div>

      <div className="mt-3">
        <Link href={`/events/${event.id}`} className="font-heading text-lg italic font-semibold leading-tight hover:text-primary">
          {event.title}
        </Link>
