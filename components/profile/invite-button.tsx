...
type InviteKind = { type: "live"; streamId: string } | { type: "event"; event: { id: string; title: string } } | { type: "chat" };
...
function defaultMessage(kind: InviteKind, origin: string): string {
  ...
  return `You're invited to "${kind.event.title}"! ${origin}/events/${kind.event.id}`;
}
    ...
    const kind: InviteKind =
      selection === "live" && options.liveStreamId
        ...
        : selection === "chat"
          ...
          : {
              type: "event",
              event: options.events.find((event) => event.id === selection) ?? { id: selection, title: "my event" },
            };
                  ...
                  {options?.events.map((event) => (
                    <label key={event.id} className="flex items-center gap-2 text-sm">
                      <input
                        ...
                        onChange={() => setSelection(event.id)}
                      />
                      {event.title}{" "}
