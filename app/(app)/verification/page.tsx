...
/** Any of the three request types satisfies the same unified identity check (see
 * lib/verification.ts's hasIdentityOnFile), so a provider who hasn't submitted yet
 * just needs a sensible default to submit under based on their account type. */
const DEFAULT_REQUEST_TYPE: Partial<Record<string, VerificationRequestType>> = {
  CREATOR: "creator",
  PAIR: "creator",
    ...
    <div className="flex flex-col gap-4 md:gap-6">
      ...
      {isProvider ? (
        <VerificationRequestCard
          requestType={DEFAULT_REQUEST_TYPE[profile.profileType] ?? "creator"}
        ...
        <div className="flex items-start gap-3 rounded-2xl border border-dashed border-border/60 bg-card p-4 shadow-sm md:rounded-xl md:shadow-none">
          ...
          <div className="text-sm text-muted-foreground">
            <p>
              Identity verification is for provider accounts hosting events, listing services, or posting
              premium content.
            </p>
