...
export const config = {
  matcher: [
    ...
    // events, and service listings — these need to render for logged-out
    ...
    // routes (`/profile/edit`, `/events/new`, `/events/manage`, `/services/new`,
    // `/services/bookings`), which stay auth-gated as before.
    "/((?!login|signup|landing|blog|help|offline|api/auth|api/signup|_next/static|_next/image|icons/.*|profile/(?!edit(?:/|$))[^/]+|posts/[^/]+|events/(?!new(?:/|$)|manage(?:/|$))[^/]+|services/(?!new(?:/|$)|bookings(?:/|$))[^/]+|.*\\.(?:png|jpe?g|gif|svg|webp|avif|ico|json|js|css|woff2?|txt|xml)$).+)",
  ],
};
